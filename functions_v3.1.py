    ...
    def runAutomation( self ):
        ...
        if self.getCodePath() == "":
            ...
            if status is None:
                ...
                cancel_button.config( state = "disabled", bg = colors[1], fg = get_contrast_color( colors[1] ) )
                run_button.config( state = "normal", bg = colors[2], fg = get_contrast_color( colors[2] ) )
...
def get_contrast_color( background_color ):
    ...
    # Calculate the luminance (perceived brightness) using the relative luminance formula
    luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255

    # Return white if the background is too dark, else return black
...
def pythonComboboxSelected( event ):
    ...
    if selection == "":
        ...
        browse_button.config( state = "disabled", bg = colors[1], fg = get_contrast_color( colors[1] ) )
        ...
        browse_button.config( state = "normal", bg = colors[2], fg = get_contrast_color( colors[2] ) )
...
def requiredFilesCheck( uploads, d ):
    ...
    if ((d == {}) or (d == None)) and (Auto.getName() != ""):
        ...
        run_button.config( state = "disabled", bg = colors[1], fg = get_contrast_color( colors[1] ) )
        upload_checkmark.config( image = "" )
    ...
    if run_button.cget( "text" ) == "Launch":
        ...
        if Auto.getCodePath() == "":
            ...
            vOutputPath = (fr"{os.environ['USERPROFILE']}\OneDrive - Singtel\Digital Team\Automations\- Logging\3-GUI"
                        ...
                        tzinfo=None) + datetime.timedelta(hours=8)).strftime("%Y-%m-%d %H%M-%S.%f")
                           + " ; " + socket.gethostname()
                           + " ; " + os.environ["username"]
                           + ".txt")
            ...
            cancel_button.config( state = "normal", bg = colors[2], fg = get_contrast_color( colors[2] ) )
            run_button.config( state = "disabled", bg = colors[1], fg = get_contrast_color( colors[1] ) )
...
class App:
    ...
    def setButton( self, button, canvas ):
        ...
        Label( canvas, text = self.getName(), bg = colors[2], fg = get_contrast_color( colors[2] ) ).pack( side = TOP )
...
def drawAppsFrame():
    ...
    refresh_apps_button = Button( pwrapp_frame, text = "Refresh" , command = refreshApps, bg = colors[2], fg = get_contrast_color( colors[2] ) )
    ...
    app_canvas.bind("<Configure>", onAppResize)
...
def displayApps( df ):
    ...
    for i in range(df.shape[0]):
        ...
        appname = Label( temp_canvas, text = a.getName(), bg = colors[2], fg = get_contrast_color( colors[2] ) )
...
def refreshDashboards():

    global dash_canvas
    global dash_scrollbar
    global refresh_dashboards_button
    global l

    dash_canvas.destroy()
    dash_scrollbar.destroy()
    refresh_dashboards_button.destroy()
    l = drawDashboardsFrame()

    return None



def refreshDashboardsShortcut( event ):
...
def drawDashboardsFrame():

    global dash_canvas
    global dash_scrollbar
    global refresh_dashboards_button
    global dash_canvas_width
    global frame

    dash_canvas = Canvas( dshbrd_frame, bg = colors[2] , highlightthickness = 0 )
    dash_scrollbar = ttk.Scrollbar( dshbrd_frame, orient = "vertical", command = dash_canvas.yview )
    refresh_dashboards_button = Button( dshbrd_frame, text = "Refresh" , command = refreshDashboards, bg = colors[2] )
    dash_canvas.config( yscrollcommand = dash_scrollbar.set )

    refresh_dashboards_button.pack( side = BOTTOM, anchor = E )
    dash_scrollbar.pack( side = RIGHT, fill = Y )
    dash_canvas.pack( side = LEFT, expand = YES, fill = BOTH )

    # Create a frame to contain the buttons
    frame = Frame( dash_canvas, bg = colors[2] )
    dash_canvas.create_window( (0, 0), window = frame, anchor = "nw" )

    dash_list = displayDashboards( dshbrdautomations )

    dash_canvas.bind("<Configure>", onDashResize)
    frame.update_idletasks()
    dash_canvas.config( scrollregion = dash_canvas.bbox("all") )
    dash_canvas.bind( "<MouseWheel>", onCanvasScroll )
    frame.bind("<MouseWheel>", onCanvasScroll)
...
def onCanvasScroll(event):
    ...
    dash_canvas.yview_scroll(-1 * (event.delta // 120), "units")
...
def onDashResize(event):

    global dash_canvas_width
    global dash_canvas

    dash_canvas_width = event.width
    dash_canvas.configure( scrollregion = dash_canvas.bbox( "all" ) )
...
def displayDashboards( df ):

    global dash_canvas_width

    num_columns = 5
    gridspace   = (dash_canvas_width // num_columns) - 30
    ...
    for i in range(df.shape[0]):
        ...
        temp_canvas.grid( row = a.getRow(), column = a.getCol() )
        b.pack( side = TOP, padx = 10, pady = (20, 0) )
        dashname = Label( temp_canvas, text = a.getName(), bg = colors[2] )
        dashname.bind("<MouseWheel>", onCanvasScroll)
        dashname.pack( side = TOP, anchor = "n" )
